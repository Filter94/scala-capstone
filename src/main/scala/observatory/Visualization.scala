...
object Visualization {
  ...
  /**
    ...
    */
  def predictTemperature(temperatures: Iterable[(Location, Temperature)], location: Location, P: Double = 3.0): Temperature = {
...
//  Spark implementation. Scalable and cool but extremely inefficient. Just for sport.
  ...
  def predictTemperaturesSpark(temperatures: Dataset[TempByLocation],
    ...
    temperatures
      .crossJoin(locations)
      .map { row =>
        (Location(row.getAs(2), row.getAs(3)), Location.fromRow(row, 0), row.getAs[Temperature](1))
      }
      .groupByKey(_._1)
      .agg(interpolatedTemp().alias("temp").as[Temperature])
      .select($"key".as("location").as[Location], $"temp".as[Temperature])
  }
